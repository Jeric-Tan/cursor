...
const ENABLE_RAG = process.env.ENABLE_RAG !== "false"; // default: enabled
...
export async function handleChat(req, res) {
  try {
    ...
    // Respect global flag and request flag
    const wantRAG = ENABLE_RAG && useRAG;
    if (wantRAG && profile.is_ego_ready) {
      // Use RAG for context-aware responses
      try {
        console.log('[RAG] Initializing RAG for session:', sessionId);
        ...
        console.log('[RAG] Querying with message:', message);
        ...
        console.log('[RAG] Response generated with', sources.length, 'sources');
      } catch (ragError) {
      ...
      console.log('[LLM] Using standard LLM (RAG disabled or ego not ready)');
      const systemPrompt = profile.master_prompt || "You are a helpful AI assistant.";
      textResponse = await generateResponse(systemPrompt, history, message);
    }
